...
## #56 「7 月之后的营业额」被当成问未来（L3 时间）

- **现象**：`7 月之后的营业额`、`7月以后卖了多少单` 只答了 7 月一个月，另外还混进一段 2026-09-02 至 09-08 的“未来”区间。`8 月 15 日之后` 只查了 8 月 15 日当天，`6 月起/6 月以来` 只查了 6 月。“今天”是 2026-09-01，7 月之后到 8 月 31 日都是过去，照常查数就行；起点已经过了数据末尾（`9 月之后`、`8 月 31 日之后`）才是问未来。
- **假设**：“之后/以后”被当成了“以后会怎样”那种指向未来的说法，没有看出它挂在一个具体时间上。
- **验证**：`timeparse.RELATIVE_FUTURE` 里有“以后”“之后”，只要句子里出现就追加一段从明天开始的区间。“起/以来”没有任何处理，时间还是按孤立的“6 月”解析。`9 月之后` 虽然也拒答了，但那是因为按 9 月本身判的。
- **根因**：`starter/kbqa/timeparse.py` 不认“时间 + 之后/起”这种只有起点的区间；`starter/kbqa/planner.py` 也没有把这种区间的终点截到数据末尾。
- **修复**：本次提交。
  - 新增 `_AFTER`，匹配紧跟在月、日、周后面的之后、以后、往后、起、以来、开始，解析为从这个时间往后到今天的区间（`TimeSpec.open_ended`）。这种情况不再当成未来。
  - 和已有的“7 月之前 = 6 月 30 日以前”对称：“之后/以后/往后”不含这个时间本身，“起/以来/开始”含。“X 之后到 Y”的终点是 Y。
  - planner 把终点截到数据末尾；起点已经过了数据末尾的，由 `_check_period` 如实拒答，拒答文案改为“2026-10-01 以后没有任何数据”。
  - 不挂在具体时间上的“以后”（“以后会员充值还送吗”）、下个月、明天仍按未来处理，不受影响。
- **回归测试**：`tests/test_chat.py::test_after_named_time_is_past`（5 例，修复前全部失败）和 `test_after_data_end_is_future`（2 例；修复前 `8 月 31 日之后卖了多少单` 答成了 8 月 31 日当天的数据）。

## #57 追问「那之后呢」被当成问未来（L3 多轮）

- **现象**：先问 `S01 7 月营业额`，再追问 `那之后呢`，回答是「2026-09-02 至 2026-09-08 没有任何数据」。上一轮是 7 月，“之后”指 8 月 1 日到数据末尾，是过去的事。
- **假设**：#56 只处理了“之后”紧跟在具体时间后面的情况。追问里的“之后”挂在上一轮的时间上，句子里自己没有时间，于是又落回了 `RELATIVE_FUTURE`。
- **验证**：还原后的问句是「之后 S01营业额」，上一轮的“7月”在还原时已经去掉。`parse_time` 只看到一个孤立的“之后”，就按未来一周处理。
- **根因**：`starter/kbqa/followup.py` 的 `inherit_time` 只会从上一轮补全“8 号”“这两个月”“那一周”，不会补“那之后”。
- **修复**：本次提交。
  - 整句只有「(那/这)之后/以后/往后(呢)」时，区间取上一轮时间结束的次日到现在，由 planner 截到数据末尾（沿用 #56 的 `open_ended`）。门店、指标照常沿用上一轮。
  - 上一轮已经是 8 月时，起点 9 月 1 日过了数据末尾，照常按未来拒答。
  - 顺带修正 #56 拒答文案里多出的空格（「2026-10-01 以后 没有」）。
- **回归测试**：`tests/test_chat.py::test_follow_up_after_previous_window`（3 种说法，修复前全部答成未来）。

## #58 引用把列表相邻几条连着引出来（L3 文档问答）

- **现象**：`台风天几点前闭店？` 引用的是一大段，把 KB-026 通知里第 1 条「14:00 提前闭店」和第 2、3 条一起引了出来，还多引了一条 KB-062 的周末延长营业。`员工折扣是几折？`、`顾客反馈最多的是什么问题？` 也是整串列表一起引。
- **假设**：句子单元本来就是几条粘在一起的，也就是切块时把分行弄丢了。
- **验证**：打出 KB-026 的句子单元，列表各条之间没有换行，`units._lines_of` 按行拆不开；`_with_lead` 又按同一个 `line_id` 把前面的几条拼了进来。
- **根因**：分两层。
  - `starter/kbqa/chunker.py` 的 `_text_chunks` 在「。」和「\n」处都断开，于是「。」后面的换行单独成了一段空白。这段空白被当成空句丢掉，列表各条、小标题就连成了一行。
  - 各条分开以后，又暴露出第二个问题。「几点」会给检索式补上「营业时间 闭店 延长」，挑句子时这些扩写词和问句自己的词同等计分。第 3 条「恢复正常营业，按标准营业时间开店」很短，又靠标题沾到了「台风」「闭店」，结果压过了真正写着 14:00 的第 1 条。原来几条粘在一起时，这个问题被掩盖了。
- **修复**：本次提交。
  - 空白段接回上一句，不再丢弃，正文一字不丢，分行照旧保留。`CHUNKER_VERSION` 升到 `chunker-5`，让索引缓存失效。
  - planner 把扩写词单独记进 `slots["expansion"]`。`DocFacts.rank` 挑句子时，只要文档里有句子直接写到问句自己的词，就只按问句自己的词挑；一句都没写到时（「S03 几点开门」，原文只写「标准营业时间」）才让扩写词计分。扩写词在检索找文档时照常起作用。
  - 顺带变好的：`S02 营业时间是几点到几点` 原来只引了 KB-023 的 618 活动句，现在第一条是门店档案里的标准营业时间。
  - 还没修的：`顾客反馈最多` 现在引的是第 4 条「没有集中性问题」，原来引的是第 3 条「好评最多」，两个都不对，属于语义问题，留到后面处理。另外还有多出来的第二条引用（KB-011、KB-023、KB-060）。
- **回归测试**：
  - `tests/test_retrieval.py::test_chunker_keeps_line_breaks`
  - `tests/test_chat.py::test_list_item_quoted_alone`
  - 修复前这两个都失败。
